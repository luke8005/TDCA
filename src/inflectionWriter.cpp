...
/**
	...
	std::regex up1 ("d:\\(Intercept\\)+\\s*+(\\d*\\.\\d*e\\+\\d*)+.*");
	std::regex un1 ("d:\\(Intercept\\)+\\s*+(-\\d*\\.\\d*e\\+\\d*)+.*");
	std::regex up2 ("d:\\(Intercept\\)+\\s*+(\\d*\\.\\d*e\\-\\d*)+.*");
	std::regex un2 ("d:\\(Intercept\\)+\\s*+(-\\d*\\.\\d*e\\-\\d*)+.*");
	std::regex up3 ("d:\\(Intercept\\)+\\s*+(\\d*\\.\\d*)+.*");
	std::regex un3 ("d:\\(Intercept\\)+\\s*+(-\\d*\\.\\d*)+.*");
	std::regex ue (".*+(d:\\(Intercept\\) ERROR)+.*");

	i_matchCount = 0;
	std::getline(inFile, s_line);
	while (std::getline(inFile, s_line)) {
		if (std::regex_match (s_line,up1)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,up1);
			std::string s_line = sm[1];
			std::cout << std::fixed;
			double d = std::stod(s_line);
			drcUpperAsymVector[i_matchCount] = std::to_string(d);
			i_matchCount++;
		} else if (std::regex_match (s_line,un1)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,un1);
			std::string s_line = sm[1];
			std::cout << std::fixed;
			double d = std::stod(s_line);
			drcUpperAsymVector[i_matchCount] = std::to_string(d);
			i_matchCount++;
		} else if (std::regex_match (s_line,up2)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,up2);
			std::string s_line = sm[1];
			std::cout << std::fixed;
			double d = std::stod(s_line);
			drcUpperAsymVector[i_matchCount] = std::to_string(d);
			i_matchCount++;
		} else if (std::regex_match (s_line,un2)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,un2);
			std::string s_line = sm[1];
			std::cout << std::fixed;
			double d = std::stod(s_line);
			drcUpperAsymVector[i_matchCount] = std::to_string(d);
			i_matchCount++;
		} else if (std::regex_match (s_line,up3)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,up3);
			std::string s_line = sm[1];
			drcUpperAsymVector[i_matchCount] = s_line;
			i_matchCount++;
		} else if (std::regex_match (s_line,un3)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,un3);
			std::string s_line = sm[1];
			drcUpperAsymVector[i_matchCount] = s_line;
			i_matchCount++;
		} else if (std::regex_match (s_line,ue)) {
			std::smatch sm;  
			std::regex_match (s_line,sm,ue);
			std::string s_line = "NaN";
			drcUpperAsymVector[i_matchCount] = s_line;
			i_matchCount++;
		} else {
...
/**
	std::ifstream inFile("drcInflections.txt");	// ADDED
	// get residuals
	inFile.clear();
	inFile.seekg(0, ios::beg);
